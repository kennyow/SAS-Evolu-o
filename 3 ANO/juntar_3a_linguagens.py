...
for row in range(2, new_sheet.max_row+1):
    ...
    valor_coluna_d_abaixo = new_sheet.cell(row=row + 1, column=4).value

    # Verifique se a célula atual é 'Inglês' e a célula abaixo é 'Espanhol'
    if valor_coluna_d == 'Inglês' and valor_coluna_d_abaixo == 'Espanhol':
        rows_to_delete.append(row + 1)  # Adicione a linha com 'Espanhol' à lista de exclusão
...
for row in range(1, saved_sheet.max_row, 5):
...
Matérias01 = ['Arte', 'Filosofia', 'Geografia', 'História', 'Sociologia']
